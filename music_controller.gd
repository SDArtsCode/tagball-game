...
func _process(delta):
	...
	if phase == 2:
		if get_volume("Team11") < 1.0:
			...
			change_volume("Team12", TRANSITION_SPEED*delta)
			change_volume("Team13", TRANSITION_SPEED*delta)
			set_volume("Team2", 1-get_volume("Team11"))
			set_volume("Intro", 1-get_volume("Team11"))
		
	elif get_volume("Team11") > 0.0:
		change_volume("Team11", -TRANSITION_SPEED*delta)
		change_volume("Team12", -TRANSITION_SPEED*delta)
		change_volume("Team13", -TRANSITION_SPEED*delta)
...
func reset():
	rng.randomize()
	$Transition.stream.loop = true


var playing_intro: bool = false
func play_intro():
	$Transition.stream.loop = false
	phase = 1
	set_volume("Transition", 1.0)
